...
do
    bin/driver -f trace_incast_144_1024B.csv.processed -a 1 -s $sram -i 1 -v 120 -x 5 -y 5 >> experiment_incast_1024.txt
done
...
do
    ...
    python scripts/belady.py -f ./out -n $sram -x 145 -s 1000 -d 2 >> best_case_incast_1024.txt
    rm -r ./out/*.*
done

echo "Running Permutation 1024B"
...
do
    ...
    do
        bin/driver -f trace_permutation_144_1024B.csv.processed -a 1 -s $sram -i 1 -u 5 -l $load >> experiment_permutation_1024.txt
    done
    ...
    do
        ...
        rm -r ./out/*.out
        rm -r ./out/*.timeseries.csv
        python scripts/belady.py -f ./out -n $sram -x 21 -s 10000 -d 2 >> best_case_permutation_1024.txt
        rm -r ./out/*.*
    done
done
