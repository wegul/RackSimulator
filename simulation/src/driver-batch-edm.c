    ...
    while (1)
    {
        ...
        for (int i = 0; i < MAX_FLOW_ID; i++)
        {
            ...
            if (ntf->remainingReqLen <= 0)
            {
                ntf->reqFlowID = -1;
                ntf->curQuota = -1;
                ntf->remainingReqLen = INT32_MAX - 1;
            }
        }
        ...
        */
        qsort(tor->notif_queue, min(tor->ntf_cnt, MAX_FLOW_ID), sizeof(notif_t), cmp_ntf);
        // From head to tail, assign grant to each notif.
        for (int i = 0; i < min(tor->ntf_cnt, MAX_FLOW_ID); i++)
        {
            notif_t ntf = tor->notif_queue[i];
        ...
        for (int i = 0; i < NUM_OF_NODES; ++i)
        {
            ...
            {
                // Only 2 cases: Granted(keep) / Waiting(shut)
