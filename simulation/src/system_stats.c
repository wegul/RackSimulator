...
#ifdef PRINT_STATS
    printf("Network tput (recvd) [%d] = %0.3lf Gbps %0.3lf -- (%lf, %ld)\n\n",
            count, network_tput,
            (double)data_recvd/(double)(num_of_pkt_injected * pkt_size * 8),
            data_recvd, (num_of_pkt_injected*pkt_size*8));
    fflush(stdout);
#endif
            ...
            int8_t tor_downstream_queue)
{
    ...
    for (int64_t j = 0; j < LEN; ++j) {
        ...
        //    printf("agg_queue_histogram[%ld] = %ld\n", j, agg_histogram[j]);
    }
