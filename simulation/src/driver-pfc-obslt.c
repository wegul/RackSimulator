        ...
        for (int i = 0; i < NUM_OF_NODES; ++i)
        {
            ...
            if (node->current_flow == NULL) // Current is NULL, choose the candidate
            {
                if (minidx >= 0)
                {
                    ...
                    // printf("node %d PeekSelect flow %d, curr: %d \n", node->node_index, node->current_flow->flow_id, curr_timeslot);
                }
            }
            ...
            if (flow)
            {
                ...
                if (flow->flowType == NET_TYPE && nodePFCPause[node_index][flow->dst] == 0)
                // NetFlow: Send packets from this flow until cwnd is reached or the flow runs out of bytes to send
                {
                    ...
                    if (flow_bytes_remaining > 0)
                    {
                        ...
                        // Pfabric: add remaining flow size.
                        pkt->reqLen = flow_bytes_remaining;
                        ...
                        if (flow->bytes_sent >= flow->flow_size_bytes)
                        {
                            node->current_flow = NULL;
                        }
                    }
                    ...
                    else
                    {
                        if (pkt->pktType == NET_TYPE)
                        {
                            ...
                            if (pkt->seq_num - node->ack_num[pkt->flow_id] > ETH_MTU)
                            {
                                ...
                                ack->reqLen = 0; // pfabric: highest priority ack
