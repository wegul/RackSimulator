...
class Flow:
    def __init__(self, elmts):
        self.flowId = int(elmts.iloc[0])
        self.flowType = int(elmts.iloc[1])
        self.src = int(elmts.iloc[2])
        self.dst = int(elmts.iloc[3])
        self.flowSize = int(elmts.iloc[4])
        self.reqLen = int(elmts.iloc[5])
        self.createTime = int(elmts.iloc[6])
...
def main():
    ...
    for i in range(0, MAX_FLOW_ID):
        ...
        idealFCT_list.append(flow.idealStart-flow.createTime+33)

    trace['IdealStart'] = idealStart_list
    trace['IdealFCT'] = idealFCT_list
