...
for dir in *; do
    if [[ -d "$dir" ]]; then
        ...
        for file in proced*.csv; do
            if [ -f "$file" ]; then
                if [[ ${dir} == nomat* ]]; then
                    echo "~/Desktop/RackSimulator/simulation/bin/driver-chunk-nomatching -f $file -n ${para_n}  > log-${file}-${para_n}.txt"
                    ~/Desktop/RackSimulator/simulation/bin/driver-chunk-nomatching -f $file -n ${para_n} >log-${file}-${para_n}.txt
                else
